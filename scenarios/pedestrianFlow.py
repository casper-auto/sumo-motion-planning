...
def get_options():
    ...
    if options.trip is not None:
        ...
        if len(options.trip) == 0 or len(options.trip) % 2 != 0:
            sys.exit("trip must contain of exactly multiples of two edges")
...
def write_ped(f, index, options, depart):
    ...
    if options.trip is not None:
        edges = 'from="%s" to="%s"' % options.trip[(2*index)%len(options.trip) : (2*index+1)%len(options.trip)+1]
    else:
