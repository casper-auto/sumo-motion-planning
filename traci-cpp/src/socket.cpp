...
int Socket::getFreeSocketPort() {
  ...
  if (bind(sock, (struct sockaddr *)&self, address_len) < 0)
    BailOnSocketError(
        "tcpip::Socket::getFreeSocketPort() Unable to bind socket");
...
#ifdef WIN32
  if (server_socket_ == -1 && socket_ == -1 && init_windows_sockets_ &&
      instance_count_ == 0)
    WSACleanup();
...
// ----------------------------------------------------------------------
int Socket::port() { return port_; }

// ----------------------------------------------------------------------
bool Socket::datawaiting(int sock) const {
  fd_set fds;
  FD_ZERO(&fds);
  ...
  if (r < 0) BailOnSocketError("tcpip::Socket::datawaiting @ select");
...
// ----------------------------------------------------------------------
bool Socket::atoaddr(std::string address, struct sockaddr_in &addr) {
  int status;
  ...
  if ((status = getaddrinfo(address.c_str(), nullptr, &hints, &servinfo)) !=
      0) {
    return false;
  }
...
// ----------------------------------------------------------------------
Socket *Socket::accept(const bool create) {
  if (socket_ >= 0) return nullptr;
  ...
  if (server_socket_ < 0) {
    ...
    {
#ifdef WIN32
      // setsockopt(server_socket_, SOL_SOCKET, SO_REUSEADDR, (const
    ...
    if (bind(server_socket_, (struct sockaddr *)&self, sizeof(self)) != 0)
      BailOnSocketError(
          "tcpip::Socket::accept() Unable to create listening socket");

    // Make it a "listening socket"
    if (listen(server_socket_, 10) == -1)
      BailOnSocketError(
          "tcpip::Socket::accept() Unable to listen on server socket");
  ...
  socket_ = static_cast<int>(
      ::accept(server_socket_, (struct sockaddr *)&client_addr, &addrlen));
...
void Socket::set_blocking(bool blocking) {
  ...
  if (server_socket_ > 0) {
    ...
    if (ioctlsocket(server_socket_, FIONBIO, &NonBlock) == SOCKET_ERROR)
      BailOnSocketError(
          "tcpip::Socket::set_blocking() Unable to initialize non blocking "
          "I/O");
#else
...
void Socket::connect() {
  ...
  if (socket_ < 0) BailOnSocketError("tcpip::Socket::connect() @ socket");
...
#ifdef WIN32
    int bytesSent =
        ::send(socket_, (const char *)bufPtr, static_cast<int>(numbytes), 0);
#else
    int bytesSent = ::send(socket_, bufPtr, numbytes, 0);
#endif
    if (bytesSent < 0) BailOnSocketError("send failed");
...
// ----------------------------------------------------------------------
size_t Socket::recvAndCheck(unsigned char *const buffer,
                            std::size_t len) const {
#ifdef WIN32
  const int bytesReceived =
      recv(socket_, (char *)buffer, static_cast<int>(len), 0);
#else
...
// ----------------------------------------------------------------------
void Socket::printBufferOnVerbose(const std::vector<unsigned char> buffer,
...
// ----------------------------------------------------------------------
std::vector<unsigned char> Socket::receive(int bufSize) {
  std::vector<unsigned char> buffer;

  if (socket_ < 0) connect();

  if (!datawaiting(socket_)) return buffer;
...
bool Socket::receiveExact(Storage &msg) {
  ...
  // contiguously. Explicitly &buffer[n] == &buffer[0] + n for 0 <= n <
  // buffer.size().
...
// ----------------------------------------------------------------------
bool Socket::has_client_connection() const { return socket_ >= 0; }

// ----------------------------------------------------------------------
bool Socket::is_blocking() { return blocking_; }

#ifdef WIN32
// ----------------------------------------------------------------------
std::string Socket::GetWinsockErrorString(int err) {
  switch (err) {
    case 0:
      return "No error";
    case WSAEINTR:
