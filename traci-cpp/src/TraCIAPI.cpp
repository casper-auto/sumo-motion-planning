...
TraCIAPI::~TraCIAPI() { delete mySocket; }
...
void TraCIAPI::send_commandGetVariable(int domID, int varID,
                                       const std::string& objID,
                                       tcpip::Storage* add) const {
  if (mySocket == nullptr) {
...
void TraCIAPI::send_commandSetValue(int domID, int varID,
                                    const std::string& objID,
                                    tcpip::Storage& content) const {
  if (mySocket == nullptr) {
    throw tcpip::SocketException("Socket is not initialised");
  }
  tcpip::Storage outMsg;
...
void TraCIAPI::send_commandSubscribeObjectVariable(
    int domID, const std::string& objID, double beginTime, double endTime,
...
void TraCIAPI::send_commandSubscribeObjectContext(
    int domID, const std::string& objID, double beginTime, double endTime,
...
void TraCIAPI::send_commandMoveToXY(const std::string& vehicleID,
                                    const std::string& edgeID, const int lane,
  ...
  send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, MOVE_TO_XY, vehicleID,
                       content);
}

void TraCIAPI::check_resultState(tcpip::Storage& inMsg, int command,
                                 bool ignoreCommandId,
                                 std::string* acknowledgement) const {
  ...
  try {
    ...
    if (command != cmdId && !ignoreCommandId) {
      throw libsumo::TraCIException(
          "#Error: received status response to command: " + toString(cmdId) +
          " but expected: " + toString(command));
    }
  ...
  switch (resultType) {
    case RTYPE_ERR:
      throw libsumo::TraCIException(".. Answered with error to command (" +
                                    toString(command) +
                                    "), [description: " + msg + "]");
    case RTYPE_NOTIMPLEMENTED:
      throw libsumo::TraCIException(".. Sent command is not implemented (" +
                                    toString(command) +
                                    "), [description: " + msg + "]");
    case RTYPE_OK:
      if (acknowledgement != nullptr) {
        (*acknowledgement) = ".. Command acknowledged (" + toString(command) +
                             "), [description: " + msg + "]";
      }
      break;
    default:
      throw libsumo::TraCIException(".. Answered with unknown result code(" +
                                    toString(resultType) + ") to command(" +
                                    toString(command) +
                                    "), [description: " + msg + "]");
  }
  if ((cmdStart + cmdLength) != (int)inMsg.position()) {
    throw libsumo::TraCIException("#Error: command at position " +
                                  toString(cmdStart) + " has wrong length");
  }
}

int TraCIAPI::check_commandGetResult(tcpip::Storage& inMsg, int command,
                                     int expectedType,
                                     bool ignoreCommandId) const {
  ...
  if (!ignoreCommandId && cmdId != (command + 0x10)) {
    throw libsumo::TraCIException(
        "#Error: received response with command id: " + toString(cmdId) +
        "but expected: " + toString(command + 0x10));
  }
  if (expectedType >= 0) {
    ...
    if (valueDataType != expectedType) {
      throw libsumo::TraCIException("Expected " + toString(expectedType) +
                                    " but got " + toString(valueDataType));
    }
  }
  return cmdId;
}

void TraCIAPI::processGET(tcpip::Storage& inMsg, int command, int expectedType,
                          bool ignoreCommandId) const {
  check_resultState(inMsg, command, ignoreCommandId);
  check_commandGetResult(inMsg, command, expectedType, ignoreCommandId);
}

int TraCIAPI::getUnsignedByte(int cmd, int var, const std::string& id,
                              tcpip::Storage* add) {
  tcpip::Storage inMsg;
...
int TraCIAPI::getByte(int cmd, int var, const std::string& id,
                      tcpip::Storage* add) {
  tcpip::Storage inMsg;
  send_commandGetVariable(cmd, var, id, add);
...
int TraCIAPI::getInt(int cmd, int var, const std::string& id,
                     tcpip::Storage* add) {
  tcpip::Storage inMsg;
  send_commandGetVariable(cmd, var, id, add);
...
double TraCIAPI::getDouble(int cmd, int var, const std::string& id,
                           tcpip::Storage* add) {
  tcpip::Storage inMsg;
  send_commandGetVariable(cmd, var, id, add);
...
libsumo::TraCIPositionVector TraCIAPI::getPolygon(int cmd, int var,
                                                  const std::string& id,
                                                  tcpip::Storage* add) {
  tcpip::Storage inMsg;
  send_commandGetVariable(cmd, var, id, add);
...
libsumo::TraCIPosition TraCIAPI::getPosition(int cmd, int var,
                                             const std::string& id,
                                             tcpip::Storage* add) {
  tcpip::Storage inMsg;
  send_commandGetVariable(cmd, var, id, add);
...
libsumo::TraCIPosition TraCIAPI::getPosition3D(int cmd, int var,
                                               const std::string& id,
                                               tcpip::Storage* add) {
  tcpip::Storage inMsg;
  send_commandGetVariable(cmd, var, id, add);
...
std::string TraCIAPI::getString(int cmd, int var, const std::string& id,
                                tcpip::Storage* add) {
  tcpip::Storage inMsg;
  send_commandGetVariable(cmd, var, id, add);
...
std::vector<std::string> TraCIAPI::getStringVector(int cmd, int var,
                                                   const std::string& id,
                                                   tcpip::Storage* add) {
  tcpip::Storage inMsg;
  send_commandGetVariable(cmd, var, id, add);
...
libsumo::TraCIColor TraCIAPI::getColor(int cmd, int var, const std::string& id,
                                       tcpip::Storage* add) {
  tcpip::Storage inMsg;
  send_commandGetVariable(cmd, var, id, add);
...
void TraCIAPI::readVariables(tcpip::Storage& inMsg, const std::string& objectID,
                             int variableCount,
  ...
  while (variableCount > 0) {
    ...
    if (status == RTYPE_OK) {
      switch (type) {
        case TYPE_DOUBLE:
          into[objectID][variableID] =
              std::make_shared<libsumo::TraCIDouble>(inMsg.readDouble());
          break;
        case TYPE_STRING:
          into[objectID][variableID] =
              std::make_shared<libsumo::TraCIString>(inMsg.readString());
          break;
        case POSITION_2D: {
        ...
        case TYPE_INTEGER:
          into[objectID][variableID] =
              std::make_shared<libsumo::TraCIInt>(inMsg.readInt());
          break;
        ...
        default:
          throw libsumo::TraCIException("Unimplemented subscription type: " +
                                        toString(type));
      }
    } else {
      throw libsumo::TraCIException(
          "Subscription response error: variableID=" + toString(variableID) +
...
void TraCIAPI::readVariableSubscription(int cmdId, tcpip::Storage& inMsg) {
  const std::string objectID = inMsg.readString();
  const int variableCount = inMsg.readUnsignedByte();
  readVariables(inMsg, objectID, variableCount,
                myDomains[cmdId]->getModifiableSubscriptionResults());
}

void TraCIAPI::readContextSubscription(int cmdId, tcpip::Storage& inMsg) {
  ...
  while (numObjects > 0) {
    ...
    readVariables(
        inMsg, objectID, variableCount,
        myDomains[cmdId]->getModifiableContextSubscriptionResults(contextID));
    numObjects--;
  }
}

void TraCIAPI::simulationStep(double time) {
  ...
  while (numSubs > 0) {
    ...
    if (cmdId >= RESPONSE_SUBSCRIBE_INDUCTIONLOOP_VARIABLE &&
        cmdId <= RESPONSE_SUBSCRIBE_PERSON_VARIABLE) {
...
double TraCIAPI::EdgeScope::getAdaptedTraveltime(const std::string& edgeID,
  ...
  return myParent.getDouble(CMD_GET_EDGE_VARIABLE, VAR_EDGE_TRAVELTIME, edgeID,
                            &content);
}
...
double TraCIAPI::EdgeScope::getEffort(const std::string& edgeID,
  ...
  content.writeByte(TYPE_DOUBLE);
  content.writeDouble(time);
  return myParent.getDouble(CMD_GET_EDGE_VARIABLE, VAR_EDGE_EFFORT, edgeID,
                            &content);
}
...
double TraCIAPI::EdgeScope::getFuelConsumption(
...
double TraCIAPI::EdgeScope::getElectricityConsumption(
    const std::string& edgeID) const {
  return myParent.getDouble(CMD_GET_EDGE_VARIABLE, VAR_ELECTRICITYCONSUMPTION,
                            edgeID);
}

double TraCIAPI::EdgeScope::getLastStepMeanSpeed(
    const std::string& edgeID) const {
  return myParent.getDouble(CMD_GET_EDGE_VARIABLE, LAST_STEP_MEAN_SPEED,
                            edgeID);
}

double TraCIAPI::EdgeScope::getLastStepOccupancy(
    const std::string& edgeID) const {
...
double TraCIAPI::EdgeScope::getTraveltime(const std::string& edgeID) const {
  return myParent.getDouble(CMD_GET_EDGE_VARIABLE, VAR_CURRENT_TRAVELTIME,
                            edgeID);
}

int TraCIAPI::EdgeScope::getLastStepVehicleNumber(
    const std::string& edgeID) const {
  return myParent.getInt(CMD_GET_EDGE_VARIABLE, LAST_STEP_VEHICLE_NUMBER,
                         edgeID);
}

double TraCIAPI::EdgeScope::getLastStepHaltingNumber(
    const std::string& edgeID) const {
  return myParent.getInt(CMD_GET_EDGE_VARIABLE,
                         LAST_STEP_VEHICLE_HALTING_NUMBER, edgeID);
}

std::vector<std::string> TraCIAPI::EdgeScope::getLastStepVehicleIDs(
    const std::string& edgeID) const {
  return myParent.getStringVector(CMD_GET_EDGE_VARIABLE,
                                  LAST_STEP_VEHICLE_ID_LIST, edgeID);
}
...
std::string TraCIAPI::EdgeScope::getStreetName(
    const std::string& edgeID) const {
...
void TraCIAPI::EdgeScope::adaptTraveltime(const std::string& edgeID,
                                          double time, double beginSeconds,
  ...
  content.writeByte(TYPE_DOUBLE);
  content.writeDouble(time);
  myParent.send_commandSetValue(CMD_SET_EDGE_VARIABLE, VAR_EDGE_TRAVELTIME,
                                edgeID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_EDGE_VARIABLE);
}

void TraCIAPI::EdgeScope::setEffort(const std::string& edgeID, double effort,
                                    double beginSeconds,
                                    double endSeconds) const {
  tcpip::Storage content;
  content.writeByte(TYPE_COMPOUND);
  ...
  myParent.send_commandSetValue(CMD_SET_EDGE_VARIABLE, VAR_EDGE_EFFORT, edgeID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_EDGE_VARIABLE);
}
...
void TraCIAPI::EdgeScope::setMaxSpeed(const std::string& edgeID,
  ...
  myParent.send_commandSetValue(CMD_SET_EDGE_VARIABLE, VAR_MAXSPEED, edgeID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_EDGE_VARIABLE);
}
...
double TraCIAPI::GUIScope::getZoom(const std::string& viewID) const {
  return myParent.getDouble(CMD_GET_GUI_VARIABLE, VAR_VIEW_ZOOM, viewID);
}

libsumo::TraCIPosition TraCIAPI::GUIScope::getOffset(
    const std::string& viewID) const {
  return myParent.getPosition(CMD_GET_GUI_VARIABLE, VAR_VIEW_OFFSET, viewID);
}

std::string TraCIAPI::GUIScope::getSchema(const std::string& viewID) const {
  return myParent.getString(CMD_GET_GUI_VARIABLE, VAR_VIEW_SCHEMA, viewID);
}

libsumo::TraCIPositionVector TraCIAPI::GUIScope::getBoundary(
    const std::string& viewID) const {
  return myParent.getPolygon(CMD_GET_GUI_VARIABLE, VAR_VIEW_BOUNDARY, viewID);
}

void TraCIAPI::GUIScope::setZoom(const std::string& viewID, double zoom) const {
  tcpip::Storage content;
  content.writeDouble(zoom);
  myParent.send_commandSetValue(CMD_SET_GUI_VARIABLE, VAR_VIEW_ZOOM, viewID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_GUI_VARIABLE);
}

void TraCIAPI::GUIScope::setOffset(const std::string& viewID, double x,
                                   double y) const {
  tcpip::Storage content;
  content.writeUnsignedByte(POSITION_2D);
  content.writeDouble(x);
  content.writeDouble(y);
  myParent.send_commandSetValue(CMD_SET_GUI_VARIABLE, VAR_VIEW_OFFSET, viewID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_GUI_VARIABLE);
}
...
void TraCIAPI::GUIScope::setSchema(const std::string& viewID,
  ...
  myParent.send_commandSetValue(CMD_SET_GUI_VARIABLE, VAR_VIEW_SCHEMA, viewID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_GUI_VARIABLE);
}

void TraCIAPI::GUIScope::setBoundary(const std::string& viewID, double xmin,
  ...
  myParent.send_commandSetValue(CMD_SET_GUI_VARIABLE, VAR_VIEW_BOUNDARY, viewID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_GUI_VARIABLE);
}
...
void TraCIAPI::GUIScope::screenshot(const std::string& viewID,
  ...
  myParent.send_commandSetValue(CMD_SET_GUI_VARIABLE, VAR_SCREENSHOT, viewID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_GUI_VARIABLE);
}
...
void TraCIAPI::GUIScope::trackVehicle(const std::string& viewID,
  ...
  myParent.send_commandSetValue(CMD_SET_GUI_VARIABLE, VAR_TRACK_VEHICLE, viewID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_GUI_VARIABLE);
}
...
double TraCIAPI::InductionLoopScope::getPosition(
    const std::string& loopID) const {
  return myParent.getDouble(CMD_GET_INDUCTIONLOOP_VARIABLE, VAR_POSITION,
                            loopID);
}

std::string TraCIAPI::InductionLoopScope::getLaneID(
    const std::string& loopID) const {
  return myParent.getString(CMD_GET_INDUCTIONLOOP_VARIABLE, VAR_LANE_ID,
                            loopID);
}

int TraCIAPI::InductionLoopScope::getLastStepVehicleNumber(
    const std::string& loopID) const {
  return myParent.getInt(CMD_GET_INDUCTIONLOOP_VARIABLE,
                         LAST_STEP_VEHICLE_NUMBER, loopID);
}

double TraCIAPI::InductionLoopScope::getLastStepMeanSpeed(
    const std::string& loopID) const {
  return myParent.getDouble(CMD_GET_INDUCTIONLOOP_VARIABLE,
                            LAST_STEP_MEAN_SPEED, loopID);
}

std::vector<std::string> TraCIAPI::InductionLoopScope::getLastStepVehicleIDs(
    const std::string& loopID) const {
  return myParent.getStringVector(CMD_GET_INDUCTIONLOOP_VARIABLE,
                                  LAST_STEP_VEHICLE_ID_LIST, loopID);
}

double TraCIAPI::InductionLoopScope::getLastStepOccupancy(
    const std::string& loopID) const {
  return myParent.getDouble(CMD_GET_INDUCTIONLOOP_VARIABLE, LAST_STEP_OCCUPANCY,
                            loopID);
}

double TraCIAPI::InductionLoopScope::getLastStepMeanLength(
    const std::string& loopID) const {
  return myParent.getDouble(CMD_GET_INDUCTIONLOOP_VARIABLE, LAST_STEP_LENGTH,
                            loopID);
}

double TraCIAPI::InductionLoopScope::getTimeSinceDetection(
    const std::string& loopID) const {
  return myParent.getDouble(CMD_GET_INDUCTIONLOOP_VARIABLE,
                            LAST_STEP_TIME_SINCE_DETECTION, loopID);
}

std::vector<libsumo::TraCIVehicleData>
TraCIAPI::InductionLoopScope::getVehicleData(const std::string& loopID) const {
  tcpip::Storage inMsg;
  myParent.send_commandGetVariable(CMD_GET_INDUCTIONLOOP_VARIABLE,
                                   LAST_STEP_VEHICLE_DATA, loopID);
...
libsumo::TraCIPosition TraCIAPI::JunctionScope::getPosition(
    const std::string& junctionID) const {
  return myParent.getPosition(CMD_GET_JUNCTION_VARIABLE, VAR_POSITION,
                              junctionID);
}
...
std::vector<std::string> TraCIAPI::LaneScope::getAllowed(
    const std::string& laneID) const {
  return myParent.getStringVector(CMD_GET_LANE_VARIABLE, LANE_ALLOWED, laneID);
}

std::vector<std::string> TraCIAPI::LaneScope::getDisallowed(
    const std::string& laneID) const {
  return myParent.getStringVector(CMD_GET_LANE_VARIABLE, LANE_DISALLOWED,
                                  laneID);
}
...
std::vector<libsumo::TraCIConnection> TraCIAPI::LaneScope::getLinks(
    const std::string& laneID) const {
  tcpip::Storage inMsg;
  ...
  for (int i = 0; i < linkNo; ++i) {
    ...
    ret.push_back(libsumo::TraCIConnection(approachedLane, hasPrio, isOpen,
                                           hasFoe, approachedLaneInternal,
                                           state, direction, length));
  }
  return ret;
}

libsumo::TraCIPositionVector TraCIAPI::LaneScope::getShape(
    const std::string& laneID) const {
...
double TraCIAPI::LaneScope::getFuelConsumption(
    const std::string& laneID) const {
...
double TraCIAPI::LaneScope::getElectricityConsumption(
    const std::string& laneID) const {
  return myParent.getDouble(CMD_GET_LANE_VARIABLE, VAR_ELECTRICITYCONSUMPTION,
                            laneID);
}

double TraCIAPI::LaneScope::getLastStepMeanSpeed(
    const std::string& laneID) const {
  return myParent.getDouble(CMD_GET_LANE_VARIABLE, LAST_STEP_MEAN_SPEED,
                            laneID);
}

double TraCIAPI::LaneScope::getLastStepOccupancy(
    const std::string& laneID) const {
...
double TraCIAPI::LaneScope::getTraveltime(const std::string& laneID) const {
  return myParent.getDouble(CMD_GET_LANE_VARIABLE, VAR_CURRENT_TRAVELTIME,
                            laneID);
}

int TraCIAPI::LaneScope::getLastStepVehicleNumber(
    const std::string& laneID) const {
  return myParent.getInt(CMD_GET_LANE_VARIABLE, LAST_STEP_VEHICLE_NUMBER,
                         laneID);
}

int TraCIAPI::LaneScope::getLastStepHaltingNumber(
    const std::string& laneID) const {
  return myParent.getInt(CMD_GET_LANE_VARIABLE,
                         LAST_STEP_VEHICLE_HALTING_NUMBER, laneID);
}

std::vector<std::string> TraCIAPI::LaneScope::getLastStepVehicleIDs(
    const std::string& laneID) const {
  return myParent.getStringVector(CMD_GET_LANE_VARIABLE,
                                  LAST_STEP_VEHICLE_ID_LIST, laneID);
}

std::vector<std::string> TraCIAPI::LaneScope::getFoes(
    const std::string& laneID, const std::string& toLaneID) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_STRING);
  content.writeString(toLaneID);
  myParent.send_commandGetVariable(CMD_GET_LANE_VARIABLE, VAR_FOES, laneID,
                                   &content);
  tcpip::Storage inMsg;
...
std::vector<std::string> TraCIAPI::LaneScope::getInternalFoes(
    const std::string& laneID) const {
  return getFoes(laneID, "");
}

void TraCIAPI::LaneScope::setAllowed(
    const std::string& laneID,
    const std::vector<std::string>& allowedClasses) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_STRINGLIST);
  ...
  myParent.send_commandSetValue(CMD_SET_LANE_VARIABLE, LANE_ALLOWED, laneID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_LANE_VARIABLE);
}

void TraCIAPI::LaneScope::setDisallowed(
    const std::string& laneID,
    const std::vector<std::string>& disallowedClasses) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_STRINGLIST);
  ...
  myParent.send_commandSetValue(CMD_SET_LANE_VARIABLE, LANE_DISALLOWED, laneID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_LANE_VARIABLE);
}
...
void TraCIAPI::LaneScope::setMaxSpeed(const std::string& laneID,
  ...
  content.writeUnsignedByte(TYPE_DOUBLE);
  content.writeDouble(speed);
  myParent.send_commandSetValue(CMD_SET_LANE_VARIABLE, VAR_MAXSPEED, laneID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_LANE_VARIABLE);
}
...
void TraCIAPI::LaneScope::setLength(const std::string& laneID,
  ...
  myParent.send_commandSetValue(CMD_SET_LANE_VARIABLE, VAR_LENGTH, laneID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_LANE_VARIABLE);
}
...
std::vector<std::string> TraCIAPI::MeMeScope::getIDList() const {
  return myParent.getStringVector(CMD_GET_MULTIENTRYEXIT_VARIABLE,
                                  TRACI_ID_LIST, "");
}

int TraCIAPI::MeMeScope::getLastStepVehicleNumber(
    const std::string& detID) const {
  return myParent.getInt(CMD_GET_MULTIENTRYEXIT_VARIABLE,
                         LAST_STEP_VEHICLE_NUMBER, detID);
}

double TraCIAPI::MeMeScope::getLastStepMeanSpeed(
    const std::string& detID) const {
  return myParent.getInt(CMD_GET_MULTIENTRYEXIT_VARIABLE, LAST_STEP_MEAN_SPEED,
                         detID);
}

std::vector<std::string> TraCIAPI::MeMeScope::getLastStepVehicleIDs(
    const std::string& detID) const {
  return myParent.getStringVector(CMD_GET_MULTIENTRYEXIT_VARIABLE,
                                  LAST_STEP_VEHICLE_ID_LIST, detID);
}

int TraCIAPI::MeMeScope::getLastStepHaltingNumber(
    const std::string& detID) const {
  return myParent.getInt(CMD_GET_MULTIENTRYEXIT_VARIABLE,
                         LAST_STEP_VEHICLE_HALTING_NUMBER, detID);
}
...
libsumo::TraCIPosition TraCIAPI::POIScope::getPosition(
    const std::string& poiID) const {
  return myParent.getPosition(CMD_GET_POI_VARIABLE, VAR_POSITION, poiID);
}

libsumo::TraCIColor TraCIAPI::POIScope::getColor(
    const std::string& poiID) const {
...
void TraCIAPI::POIScope::setPosition(const std::string& poiID, double x,
                                     double y) const {
  tcpip::Storage content;
  content.writeUnsignedByte(POSITION_2D);
  content.writeDouble(x);
  content.writeDouble(y);
  myParent.send_commandSetValue(CMD_SET_POI_VARIABLE, VAR_POSITION, poiID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_POI_VARIABLE);
}
...
void TraCIAPI::POIScope::setColor(const std::string& poiID,
  ...
  myParent.send_commandSetValue(CMD_SET_POI_VARIABLE, VAR_COLOR, poiID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_POI_VARIABLE);
}

void TraCIAPI::POIScope::add(const std::string& poiID, double x, double y,
                             const libsumo::TraCIColor& c,
                             const std::string& type, int layer) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_COMPOUND);
  content.writeInt(4);
  content.writeUnsignedByte(TYPE_STRING);
...
double TraCIAPI::PolygonScope::getLineWidth(
    const std::string& polygonID) const {
  return myParent.getDouble(CMD_GET_POLYGON_VARIABLE, VAR_WIDTH, polygonID);
}

std::string TraCIAPI::PolygonScope::getType(
    const std::string& polygonID) const {
  return myParent.getString(CMD_GET_POLYGON_VARIABLE, VAR_TYPE, polygonID);
}

libsumo::TraCIPositionVector TraCIAPI::PolygonScope::getShape(
    const std::string& polygonID) const {
  return myParent.getPolygon(CMD_GET_POLYGON_VARIABLE, VAR_SHAPE, polygonID);
}

libsumo::TraCIColor TraCIAPI::PolygonScope::getColor(
    const std::string& polygonID) const {
...
void TraCIAPI::PolygonScope::setLineWidth(const std::string& polygonID,
  ...
  myParent.send_commandSetValue(CMD_SET_POLYGON_VARIABLE, VAR_WIDTH, polygonID,
                                content);
  tcpip::Storage inMsg;
...
void TraCIAPI::PolygonScope::setType(const std::string& polygonID,
  ...
  content.writeUnsignedByte(TYPE_STRING);
  content.writeString(setType);
  myParent.send_commandSetValue(CMD_SET_POLYGON_VARIABLE, VAR_TYPE, polygonID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_POLYGON_VARIABLE);
}

void TraCIAPI::PolygonScope::setShape(
    const std::string& polygonID,
    const libsumo::TraCIPositionVector& shape) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_POLYGON);
  if (shape.size() < 256) {
  ...
  myParent.send_commandSetValue(CMD_SET_POLYGON_VARIABLE, VAR_SHAPE, polygonID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_POLYGON_VARIABLE);
}
...
void TraCIAPI::PolygonScope::setColor(const std::string& polygonID,
  ...
  content.writeUnsignedByte(c.b);
  content.writeUnsignedByte(c.a);
  myParent.send_commandSetValue(CMD_SET_POLYGON_VARIABLE, VAR_COLOR, polygonID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_POLYGON_VARIABLE);
}
...
void TraCIAPI::PolygonScope::add(const std::string& polygonID,
                                 ...
                                 const libsumo::TraCIColor& c, bool fill,
                                 const std::string& type, int layer) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_COMPOUND);
  content.writeInt(5);
  ...
  myParent.send_commandSetValue(CMD_SET_POLYGON_VARIABLE, ADD, polygonID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_POLYGON_VARIABLE);
}
...
void TraCIAPI::PolygonScope::remove(const std::string& polygonID,
  ...
  content.writeUnsignedByte(TYPE_INTEGER);
  content.writeInt(layer);
  myParent.send_commandSetValue(CMD_SET_POLYGON_VARIABLE, REMOVE, polygonID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_POLYGON_VARIABLE);
}
...
std::vector<std::string> TraCIAPI::RouteScope::getEdges(
...
std::vector<std::string> TraCIAPI::SimulationScope::getStartingTeleportIDList()
    const {
  return myParent.getStringVector(CMD_GET_SIM_VARIABLE,
...
std::vector<std::string> TraCIAPI::SimulationScope::getEndingTeleportIDList()
    const {
  return myParent.getStringVector(CMD_GET_SIM_VARIABLE,
...
double TraCIAPI::SimulationScope::getDistance2D(double x1, double y1, double x2,
                                                double y2, bool isGeo,
                                                bool isDriving) {
  tcpip::Storage content;
  content.writeByte(TYPE_COMPOUND);
  content.writeInt(3);
  ...
  myParent.send_commandGetVariable(CMD_GET_SIM_VARIABLE, DISTANCE_REQUEST, "",
                                   &content);
  tcpip::Storage inMsg;
...
std::string TraCIAPI::TrafficLightScope::getRedYellowGreenState(
    const std::string& tlsID) const {
  return myParent.getString(CMD_GET_TL_VARIABLE, TL_RED_YELLOW_GREEN_STATE,
                            tlsID);
}
...
TraCIAPI::TrafficLightScope::getCompleteRedYellowGreenDefinition(
    const std::string& tlsID) const {
  tcpip::Storage inMsg;
  myParent.send_commandGetVariable(CMD_GET_TL_VARIABLE,
                                   TL_COMPLETE_DEFINITION_RYG, tlsID);
  ...
  for (int i = 0; i < logicNo; ++i) {
    ...
    for (int j = 0; j < phaseNumber; j++) {
      ...
      logic.phases.emplace_back(
          libsumo::TraCIPhase(duration, state, minDur, maxDur, next));
    }
    inMsg.readUnsignedByte();
...
std::vector<std::string> TraCIAPI::TrafficLightScope::getControlledLanes(
    const std::string& tlsID) const {
  return myParent.getStringVector(CMD_GET_TL_VARIABLE, TL_CONTROLLED_LANES,
                                  tlsID);
}
...
TraCIAPI::TrafficLightScope::getControlledLinks(
    const std::string& tlsID) const {
  tcpip::Storage inMsg;
  myParent.send_commandGetVariable(CMD_GET_TL_VARIABLE, TL_CONTROLLED_LINKS,
                                   tlsID);
  myParent.processGET(inMsg, CMD_GET_TL_VARIABLE, TYPE_COMPOUND);
...
std::string TraCIAPI::TrafficLightScope::getProgram(
    const std::string& tlsID) const {
...
double TraCIAPI::TrafficLightScope::getPhaseDuration(
    const std::string& tlsID) const {
  return myParent.getDouble(CMD_GET_TL_VARIABLE, TL_PHASE_DURATION, tlsID);
}

double TraCIAPI::TrafficLightScope::getNextSwitch(
    const std::string& tlsID) const {
  return myParent.getDouble(CMD_GET_TL_VARIABLE, TL_NEXT_SWITCH, tlsID);
}

void TraCIAPI::TrafficLightScope::setRedYellowGreenState(
    const std::string& tlsID, const std::string& state) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_STRING);
  content.writeString(state);
  myParent.send_commandSetValue(CMD_SET_TL_VARIABLE, TL_RED_YELLOW_GREEN_STATE,
                                tlsID, content);
  tcpip::Storage inMsg;
...
void TraCIAPI::TrafficLightScope::setPhase(const std::string& tlsID,
  ...
  myParent.send_commandSetValue(CMD_SET_TL_VARIABLE, TL_PHASE_INDEX, tlsID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_TL_VARIABLE);
}

void TraCIAPI::TrafficLightScope::setProgram(
    const std::string& tlsID, const std::string& programID) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_STRING);
  content.writeString(programID);
  myParent.send_commandSetValue(CMD_SET_TL_VARIABLE, TL_PROGRAM, tlsID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_TL_VARIABLE);
}
...
void TraCIAPI::TrafficLightScope::setPhaseDuration(const std::string& tlsID,
  ...
  myParent.send_commandSetValue(CMD_SET_TL_VARIABLE, TL_PHASE_DURATION, tlsID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_TL_VARIABLE);
}

void TraCIAPI::TrafficLightScope::setCompleteRedYellowGreenDefinition(
    const std::string& tlsID, const libsumo::TraCILogic& logic) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_COMPOUND);
  content.writeInt(5);
  content.writeUnsignedByte(TYPE_STRING);
  ...
  myParent.send_commandSetValue(CMD_SET_TL_VARIABLE, TL_COMPLETE_PROGRAM_RYG,
                                tlsID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_TL_VARIABLE);
}
...
double TraCIAPI::VehicleTypeScope::getMaxSpeed(
    const std::string& typeID) const {
  return myParent.getDouble(CMD_GET_VEHICLETYPE_VARIABLE, VAR_MAXSPEED, typeID);
}

double TraCIAPI::VehicleTypeScope::getSpeedFactor(
    const std::string& typeID) const {
  return myParent.getDouble(CMD_GET_VEHICLETYPE_VARIABLE, VAR_SPEED_FACTOR,
                            typeID);
}

double TraCIAPI::VehicleTypeScope::getSpeedDeviation(
    const std::string& typeID) const {
  return myParent.getDouble(CMD_GET_VEHICLETYPE_VARIABLE, VAR_SPEED_DEVIATION,
                            typeID);
}
...
double TraCIAPI::VehicleTypeScope::getEmergencyDecel(
    const std::string& typeID) const {
  return myParent.getDouble(CMD_GET_VEHICLETYPE_VARIABLE, VAR_EMERGENCY_DECEL,
                            typeID);
}

double TraCIAPI::VehicleTypeScope::getApparentDecel(
    const std::string& typeID) const {
  return myParent.getDouble(CMD_GET_VEHICLETYPE_VARIABLE, VAR_APPARENT_DECEL,
                            typeID);
}

double TraCIAPI::VehicleTypeScope::getImperfection(
    const std::string& typeID) const {
  return myParent.getDouble(CMD_GET_VEHICLETYPE_VARIABLE, VAR_IMPERFECTION,
                            typeID);
}
...
std::string TraCIAPI::VehicleTypeScope::getVehicleClass(
    const std::string& typeID) const {
  return myParent.getString(CMD_GET_VEHICLETYPE_VARIABLE, VAR_VEHICLECLASS,
                            typeID);
}

std::string TraCIAPI::VehicleTypeScope::getEmissionClass(
    const std::string& typeID) const {
  return myParent.getString(CMD_GET_VEHICLETYPE_VARIABLE, VAR_EMISSIONCLASS,
                            typeID);
}

std::string TraCIAPI::VehicleTypeScope::getShapeClass(
    const std::string& typeID) const {
  return myParent.getString(CMD_GET_VEHICLETYPE_VARIABLE, VAR_SHAPECLASS,
                            typeID);
}
...
double TraCIAPI::VehicleTypeScope::getMinGapLat(
    const std::string& typeID) const {
  return myParent.getDouble(CMD_GET_VEHICLETYPE_VARIABLE, VAR_MINGAP_LAT,
                            typeID);
}

double TraCIAPI::VehicleTypeScope::getMaxSpeedLat(
    const std::string& typeID) const {
  return myParent.getDouble(CMD_GET_VEHICLETYPE_VARIABLE, VAR_MAXSPEED_LAT,
                            typeID);
}

std::string TraCIAPI::VehicleTypeScope::getLateralAlignment(
    const std::string& typeID) const {
  return myParent.getString(CMD_GET_VEHICLETYPE_VARIABLE, VAR_LATALIGNMENT,
                            typeID);
}
...
libsumo::TraCIColor TraCIAPI::VehicleTypeScope::getColor(
    const std::string& typeID) const {
...
void TraCIAPI::VehicleTypeScope::setLength(const std::string& typeID,
  ...
  content.writeUnsignedByte(TYPE_DOUBLE);
  content.writeDouble(length);
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE, VAR_LENGTH,
                                typeID, content);
  tcpip::Storage inMsg;
...
void TraCIAPI::VehicleTypeScope::setMaxSpeed(const std::string& typeID,
  ...
  content.writeUnsignedByte(TYPE_DOUBLE);
  content.writeDouble(speed);
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE, VAR_MAXSPEED,
                                typeID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}

void TraCIAPI::VehicleTypeScope::setVehicleClass(
    const std::string& typeID, const std::string& clazz) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_STRING);
  content.writeString(clazz);
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE, VAR_VEHICLECLASS,
                                typeID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}
...
void TraCIAPI::VehicleTypeScope::setSpeedFactor(const std::string& typeID,
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE, VAR_SPEED_FACTOR,
                                typeID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}
...
void TraCIAPI::VehicleTypeScope::setSpeedDeviation(const std::string& typeID,
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE,
                                VAR_SPEED_DEVIATION, typeID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}

void TraCIAPI::VehicleTypeScope::setEmissionClass(
    const std::string& typeID, const std::string& clazz) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_STRING);
  content.writeString(clazz);
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE, VAR_EMISSIONCLASS,
                                typeID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}
...
void TraCIAPI::VehicleTypeScope::setWidth(const std::string& typeID,
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE, VAR_WIDTH, typeID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}
...
void TraCIAPI::VehicleTypeScope::setHeight(const std::string& typeID,
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE, VAR_HEIGHT,
                                typeID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}
...
void TraCIAPI::VehicleTypeScope::setMinGap(const std::string& typeID,
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE, VAR_MINGAP,
                                typeID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}
...
void TraCIAPI::VehicleTypeScope::setMinGapLat(const std::string& typeID,
                                              double minGapLat) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_DOUBLE);
  content.writeDouble(minGapLat);
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE, VAR_MINGAP_LAT,
                                typeID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}
...
void TraCIAPI::VehicleTypeScope::setMaxSpeedLat(const std::string& typeID,
  ...
  content.writeUnsignedByte(TYPE_DOUBLE);
  content.writeDouble(speed);
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE, VAR_MAXSPEED_LAT,
                                typeID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}

void TraCIAPI::VehicleTypeScope::setLateralAlignment(
    const std::string& typeID, const std::string& latAlignment) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_STRING);
  content.writeString(latAlignment);
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE, VAR_LATALIGNMENT,
                                typeID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}
...
void TraCIAPI::VehicleTypeScope::copy(const std::string& origTypeID,
                                      const std::string& newTypeID) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_STRING);
  content.writeString(newTypeID);
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE, COPY, origTypeID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}
...
void TraCIAPI::VehicleTypeScope::setShapeClass(const std::string& typeID,
  ...
  content.writeUnsignedByte(TYPE_STRING);
  content.writeString(clazz);
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE, VAR_SHAPECLASS,
                                typeID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}
...
void TraCIAPI::VehicleTypeScope::setAccel(const std::string& typeID,
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE, VAR_ACCEL, typeID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}
...
void TraCIAPI::VehicleTypeScope::setDecel(const std::string& typeID,
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE, VAR_DECEL, typeID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}
...
void TraCIAPI::VehicleTypeScope::setEmergencyDecel(const std::string& typeID,
  ...
  content.writeDouble(decel);
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE,
                                VAR_EMERGENCY_DECEL, typeID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}
...
void TraCIAPI::VehicleTypeScope::setApparentDecel(const std::string& typeID,
  ...
  content.writeDouble(decel);
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE,
                                VAR_APPARENT_DECEL, typeID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}

void TraCIAPI::VehicleTypeScope::setImperfection(const std::string& typeID,
                                                 double imperfection) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_DOUBLE);
  content.writeDouble(imperfection);
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE, VAR_IMPERFECTION,
                                typeID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}
...
void TraCIAPI::VehicleTypeScope::setTau(const std::string& typeID,
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE, VAR_TAU, typeID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}
...
void TraCIAPI::VehicleTypeScope::setColor(const std::string& typeID,
  ...
  content.writeUnsignedByte(c.b);
  content.writeUnsignedByte(c.a);
  myParent.send_commandSetValue(CMD_SET_VEHICLETYPE_VARIABLE, VAR_COLOR, typeID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLETYPE_VARIABLE);
}
...
double TraCIAPI::VehicleScope::getAcceleration(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_ACCELERATION,
                            vehicleID);
}
...
libsumo::TraCIPosition TraCIAPI::VehicleScope::getPosition(
    const std::string& vehicleID) const {
  return myParent.getPosition(CMD_GET_VEHICLE_VARIABLE, VAR_POSITION,
                              vehicleID);
}

libsumo::TraCIPosition TraCIAPI::VehicleScope::getPosition3D(
    const std::string& vehicleID) const {
  return myParent.getPosition3D(CMD_GET_VEHICLE_VARIABLE, VAR_POSITION3D,
                                vehicleID);
}
...
std::string TraCIAPI::VehicleScope::getRoadID(
    const std::string& vehicleID) const {
  return myParent.getString(CMD_GET_VEHICLE_VARIABLE, VAR_ROAD_ID, vehicleID);
}

std::string TraCIAPI::VehicleScope::getLaneID(
    const std::string& vehicleID) const {
...
std::string TraCIAPI::VehicleScope::getTypeID(
    const std::string& vehicleID) const {
  return myParent.getString(CMD_GET_VEHICLE_VARIABLE, VAR_TYPE, vehicleID);
}

std::string TraCIAPI::VehicleScope::getRouteID(
    const std::string& vehicleID) const {
...
std::vector<std::string> TraCIAPI::VehicleScope::getRoute(
    const std::string& vehicleID) const {
  return myParent.getStringVector(CMD_GET_VEHICLE_VARIABLE, VAR_EDGES,
                                  vehicleID);
}

libsumo::TraCIColor TraCIAPI::VehicleScope::getColor(
    const std::string& vehicleID) const {
  return myParent.getColor(CMD_GET_VEHICLE_VARIABLE, VAR_COLOR, vehicleID);
}

double TraCIAPI::VehicleScope::getLanePosition(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_LANEPOSITION,
                            vehicleID);
}
...
double TraCIAPI::VehicleScope::getLateralLanePosition(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_LANEPOSITION_LAT,
                            vehicleID);
}

double TraCIAPI::VehicleScope::getCO2Emission(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_CO2EMISSION,
                            vehicleID);
}

double TraCIAPI::VehicleScope::getCOEmission(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_COEMISSION,
                            vehicleID);
}

double TraCIAPI::VehicleScope::getHCEmission(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_HCEMISSION,
                            vehicleID);
}

double TraCIAPI::VehicleScope::getPMxEmission(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_PMXEMISSION,
                            vehicleID);
}

double TraCIAPI::VehicleScope::getNOxEmission(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_NOXEMISSION,
                            vehicleID);
}

double TraCIAPI::VehicleScope::getFuelConsumption(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_FUELCONSUMPTION,
                            vehicleID);
}

double TraCIAPI::VehicleScope::getNoiseEmission(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_NOISEEMISSION,
                            vehicleID);
}

double TraCIAPI::VehicleScope::getElectricityConsumption(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE,
                            VAR_ELECTRICITYCONSUMPTION, vehicleID);
}

double TraCIAPI::VehicleScope::getWaitingTime(const std::string& vehID) const {
...
double TraCIAPI::VehicleScope::getSlope(const std::string& vehID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_SLOPE, vehID);
}

std::string TraCIAPI::VehicleScope::getLine(const std::string& typeID) const {
  return myParent.getString(CMD_GET_VEHICLE_VARIABLE, VAR_LINE, typeID);
}

std::vector<std::string> TraCIAPI::VehicleScope::getVia(
    const std::string& vehicleID) const {
  return myParent.getStringVector(CMD_GET_VEHICLE_VARIABLE, VAR_VIA, vehicleID);
}

std::string TraCIAPI::VehicleScope::getEmissionClass(
    const std::string& vehicleID) const {
  return myParent.getString(CMD_GET_VEHICLE_VARIABLE, VAR_EMISSIONCLASS,
                            vehicleID);
}

std::string TraCIAPI::VehicleScope::getShapeClass(
    const std::string& vehicleID) const {
  return myParent.getString(CMD_GET_VEHICLE_VARIABLE, VAR_SHAPECLASS,
                            vehicleID);
}

std::vector<libsumo::TraCINextTLSData> TraCIAPI::VehicleScope::getNextTLS(
    const std::string& vehID) const {
  tcpip::Storage inMsg;
  myParent.send_commandGetVariable(CMD_GET_VEHICLE_VARIABLE, VAR_NEXT_TLS,
                                   vehID);
...
std::vector<libsumo::TraCIBestLanesData> TraCIAPI::VehicleScope::getBestLanes(
    const std::string& vehicleID) const {
  tcpip::Storage inMsg;
  myParent.send_commandGetVariable(CMD_GET_VEHICLE_VARIABLE, VAR_BEST_LANES,
                                   vehicleID);
  myParent.processGET(inMsg, CMD_GET_VEHICLE_VARIABLE, TYPE_COMPOUND);
  inMsg.readInt();
  inMsg.readUnsignedByte();
...
std::pair<std::string, double> TraCIAPI::VehicleScope::getLeader(
    const std::string& vehicleID, double dist) const {
  tcpip::Storage content;
  content.writeByte(TYPE_DOUBLE);
  content.writeDouble(dist);
  myParent.send_commandGetVariable(CMD_GET_VEHICLE_VARIABLE, VAR_LEADER,
                                   vehicleID, &content);
  tcpip::Storage inMsg;
  myParent.processGET(inMsg, CMD_GET_VEHICLE_VARIABLE, TYPE_COMPOUND);
  inMsg.readInt();  // components
...
std::pair<int, int> TraCIAPI::VehicleScope::getLaneChangeState(
    const std::string& vehicleID, int direction) const {
  tcpip::Storage content;
  content.writeByte(TYPE_INTEGER);
  content.writeInt(direction);
  myParent.send_commandGetVariable(CMD_GET_VEHICLE_VARIABLE, CMD_CHANGELANE,
                                   vehicleID, &content);
  tcpip::Storage inMsg;
  myParent.processGET(inMsg, CMD_GET_VEHICLE_VARIABLE, TYPE_COMPOUND);
  inMsg.readInt();  // components
...
int TraCIAPI::VehicleScope::getStopState(const std::string& vehicleID) const {
  return myParent.getUnsignedByte(CMD_GET_VEHICLE_VARIABLE, VAR_STOPSTATE,
                                  vehicleID);
}
...
double TraCIAPI::VehicleScope::getImperfection(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_IMPERFECTION,
                            vehicleID);
}

double TraCIAPI::VehicleScope::getSpeedFactor(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_SPEED_FACTOR,
                            vehicleID);
}

double TraCIAPI::VehicleScope::getSpeedDeviation(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_SPEED_DEVIATION,
                            vehicleID);
}

std::string TraCIAPI::VehicleScope::getVehicleClass(
    const std::string& vehicleID) const {
  return myParent.getString(CMD_GET_VEHICLE_VARIABLE, VAR_VEHICLECLASS,
                            vehicleID);
}
...
double TraCIAPI::VehicleScope::getHeight(const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_HEIGHT, vehicleID);
}

double TraCIAPI::VehicleScope::getAccumulatedWaitingTime(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE,
                            VAR_ACCUMULATED_WAITING_TIME, vehicleID);
}

double TraCIAPI::VehicleScope::getAllowedSpeed(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_ALLOWED_SPEED,
                            vehicleID);
}

int TraCIAPI::VehicleScope::getPersonNumber(
    const std::string& vehicleID) const {
  return myParent.getInt(CMD_GET_VEHICLE_VARIABLE, VAR_PERSON_NUMBER,
                         vehicleID);
}

std::vector<std::string> TraCIAPI::VehicleScope::getPersonIDList(
    const std::string& vehicleID) const {
  return myParent.getStringVector(CMD_GET_VEHICLE_VARIABLE,
                                  LAST_STEP_PERSON_ID_LIST, vehicleID);
}

double TraCIAPI::VehicleScope::getSpeedWithoutTraCI(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_SPEED_WITHOUT_TRACI,
                            vehicleID);
}

bool TraCIAPI::VehicleScope::isRouteValid(const std::string& vehicleID) const {
  return myParent.getInt(CMD_GET_VEHICLE_VARIABLE, VAR_ROUTE_VALID,
                         vehicleID) != 0;
}

double TraCIAPI::VehicleScope::getMaxSpeedLat(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_MAXSPEED_LAT,
                            vehicleID);
}

double TraCIAPI::VehicleScope::getMinGapLat(
    const std::string& vehicleID) const {
  return myParent.getDouble(CMD_GET_VEHICLE_VARIABLE, VAR_MINGAP_LAT,
                            vehicleID);
}

std::string TraCIAPI::VehicleScope::getLateralAlignment(
    const std::string& vehicleID) const {
  return myParent.getString(CMD_GET_VEHICLE_VARIABLE, VAR_LATALIGNMENT,
                            vehicleID);
}

void TraCIAPI::VehicleScope::add(
    const std::string& vehicleID, const std::string& routeID,
    const std::string& typeID, std::string depart,
    const std::string& departLane, const std::string& departPos,
    const std::string& departSpeed, const std::string& arrivalLane,
    const std::string& arrivalPos, const std::string& arrivalSpeed,
    const std::string& fromTaz, const std::string& toTaz,
    const std::string& line, int personCapacity, int personNumber) const {
  if (depart == "-1") {
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, ADD_FULL, vehicleID,
                                content);
  tcpip::Storage inMsg;
...
void TraCIAPI::VehicleScope::remove(const std::string& vehicleID,
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, REMOVE, vehicleID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::changeTarget(const std::string& vehicleID,
  ...
  content.writeUnsignedByte(TYPE_STRING);
  content.writeString(edgeID);
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, CMD_CHANGETARGET,
                                vehicleID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::changeLane(const std::string& vehicleID,
                                        int laneIndex, double duration) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_COMPOUND);
  content.writeInt(2);
  content.writeUnsignedByte(TYPE_BYTE);
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, CMD_CHANGELANE,
                                vehicleID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::changeSublane(const std::string& vehicleID,
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, CMD_CHANGESUBLANE,
                                vehicleID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::setRouteID(const std::string& vehicleID,
  ...
  content.writeUnsignedByte(TYPE_STRING);
  content.writeString(routeID);
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, VAR_ROUTE_ID,
                                vehicleID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}

void TraCIAPI::VehicleScope::setRoute(
    const std::string& vehicleID, const std::vector<std::string>& edges) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_STRINGLIST);
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, VAR_ROUTE, vehicleID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::rerouteTraveltime(const std::string& vehicleID,
                                               bool currentTravelTimes) const {
  if (currentTravelTimes) {
    // updated edge weights with current network traveltimes (at most once per
    // simulation step)
    std::vector<std::string> edges = myParent.edge.getIDList();
    for (std::vector<std::string>::iterator it = edges.begin();
         it != edges.end(); ++it) {
  ...
  content.writeUnsignedByte(TYPE_COMPOUND);
  content.writeInt(0);
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE,
                                CMD_REROUTE_TRAVELTIME, vehicleID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::moveTo(const std::string& vehicleID,
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, VAR_MOVE_TO,
                                vehicleID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::moveToXY(const std::string& vehicleID,
                                      ...
                                      const double angle,
                                      const int keepRoute) const {
  myParent.send_commandMoveToXY(vehicleID, edgeID, lane, x, y, angle,
                                keepRoute);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::slowDown(const std::string& vehicleID,
                                      double speed, double duration) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_COMPOUND);
  content.writeInt(2);
  content.writeUnsignedByte(TYPE_DOUBLE);
  ...
  content.writeUnsignedByte(TYPE_DOUBLE);
  content.writeDouble(duration);
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, CMD_SLOWDOWN,
                                vehicleID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::openGap(const std::string& vehicleID,
                                     double newTau, double duration,
                                     double changeRate, double maxDecel) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_COMPOUND);
  if (maxDecel > 0) {
    content.writeInt(4);
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, CMD_OPENGAP,
                                vehicleID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::setSpeed(const std::string& vehicleID,
  ...
  content.writeUnsignedByte(TYPE_DOUBLE);
  content.writeDouble(speed);
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, VAR_SPEED, vehicleID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::setSpeedMode(const std::string& vehicleID,
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, VAR_SPEEDSETMODE,
                                vehicleID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::setStop(const std::string vehicleID,
                                     ...
                                     const double endPos, const int laneIndex,
                                     const double duration, const int flags,
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, CMD_STOP, vehicleID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::setType(const std::string& vehicleID,
  ...
  content.writeUnsignedByte(TYPE_STRING);
  content.writeString(typeID);
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, VAR_TYPE, vehicleID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::setSpeedFactor(const std::string& vehicleID,
  ...
  content.writeUnsignedByte(TYPE_DOUBLE);
  content.writeDouble(factor);
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, VAR_SPEED_FACTOR,
                                vehicleID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::setMaxSpeed(const std::string& vehicleID,
  ...
  content.writeUnsignedByte(TYPE_DOUBLE);
  content.writeDouble(speed);
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, VAR_MAXSPEED,
                                vehicleID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::setColor(const std::string& vehicleID,
  ...
  content.writeUnsignedByte(c.b);
  content.writeUnsignedByte(c.a);
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, VAR_COLOR, vehicleID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::setLine(const std::string& vehicleID,
  ...
  content.writeUnsignedByte(TYPE_STRING);
  content.writeString(line);
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, VAR_LINE, vehicleID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::setVia(const std::string& vehicleID,
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, VAR_VIA, vehicleID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::setSignals(const std::string& vehicleID,
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, VAR_SIGNALS,
                                vehicleID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::setRoutingMode(const std::string& vehicleID,
  ...
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, VAR_ROUTING_MODE,
                                vehicleID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::setShapeClass(const std::string& vehicleID,
  ...
  content.writeUnsignedByte(TYPE_STRING);
  content.writeString(clazz);
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, VAR_SHAPECLASS,
                                vehicleID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
void TraCIAPI::VehicleScope::setEmissionClass(const std::string& vehicleID,
  ...
  content.writeUnsignedByte(TYPE_STRING);
  content.writeString(clazz);
  myParent.send_commandSetValue(CMD_SET_VEHICLE_VARIABLE, VAR_EMISSIONCLASS,
                                vehicleID, content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_VEHICLE_VARIABLE);
}
...
libsumo::TraCIPosition TraCIAPI::PersonScope::getPosition(
    const std::string& personID) const {
  return myParent.getPosition(CMD_GET_PERSON_VARIABLE, VAR_POSITION, personID);
}

libsumo::TraCIPosition TraCIAPI::PersonScope::getPosition3D(
    const std::string& personID) const {
  return myParent.getPosition3D(CMD_GET_PERSON_VARIABLE, VAR_POSITION3D,
                                personID);
}
...
double TraCIAPI::PersonScope::getLanePosition(
    const std::string& personID) const {
  return myParent.getDouble(CMD_GET_PERSON_VARIABLE, VAR_LANEPOSITION,
                            personID);
}

libsumo::TraCIColor TraCIAPI::PersonScope::getColor(
    const std::string& personID) const {
...
std::vector<std::string> TraCIAPI::PersonScope::getEdges(
    const std::string& personID, int nextStageIndex) const {
  tcpip::Storage content;
  content.writeByte(TYPE_INTEGER);
  content.writeInt(nextStageIndex);
...
void TraCIAPI::PersonScope::rerouteTraveltime(
    const std::string& personID) const {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_COMPOUND);
  content.writeInt(0);
  myParent.send_commandSetValue(CMD_SET_PERSON_VARIABLE, CMD_REROUTE_TRAVELTIME,
                                personID, content);
  tcpip::Storage inMsg;
...
void TraCIAPI::PersonScope::add(const std::string& personID,
                                const std::string& edgeID, double pos,
                                double depart, const std::string typeID) {
  tcpip::Storage content;
  content.writeUnsignedByte(TYPE_COMPOUND);
  content.writeInt(4);
  content.writeUnsignedByte(TYPE_STRING);
  ...
  myParent.send_commandSetValue(CMD_SET_PERSON_VARIABLE, ADD, personID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_PERSON_VARIABLE);
}
...
void TraCIAPI::PersonScope::appendWaitingStage(const std::string& personID,
  ...
  myParent.send_commandSetValue(CMD_SET_PERSON_VARIABLE, APPEND_STAGE, personID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_PERSON_VARIABLE);
}

void TraCIAPI::PersonScope::appendWalkingStage(
    const std::string& personID, const std::vector<std::string>& edges,
    double arrivalPos, double duration, double speed,
    const std::string& stopID) {
  if (duration > 0) {
    duration *= 1000;
  }
...
void TraCIAPI::PersonScope::removeStage(const std::string& personID,
  ...
  content.writeByte(TYPE_INTEGER);
  content.writeInt(nextStageIndex);
  myParent.send_commandSetValue(CMD_SET_PERSON_VARIABLE, REMOVE_STAGE, personID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_PERSON_VARIABLE);
}
...
void TraCIAPI::PersonScope::setSpeed(const std::string& personID,
  ...
  content.writeUnsignedByte(TYPE_DOUBLE);
  content.writeDouble(speed);
  myParent.send_commandSetValue(CMD_SET_PERSON_VARIABLE, VAR_SPEED, personID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_PERSON_VARIABLE);
}
...
void TraCIAPI::PersonScope::setType(const std::string& personID,
  ...
  content.writeUnsignedByte(TYPE_STRING);
  content.writeString(typeID);
  myParent.send_commandSetValue(CMD_SET_PERSON_VARIABLE, VAR_TYPE, personID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_PERSON_VARIABLE);
}
...
void TraCIAPI::PersonScope::setLength(const std::string& personID,
  ...
  content.writeUnsignedByte(TYPE_DOUBLE);
  content.writeDouble(length);
  myParent.send_commandSetValue(CMD_SET_PERSON_VARIABLE, VAR_LENGTH, personID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_PERSON_VARIABLE);
}
...
void TraCIAPI::PersonScope::setWidth(const std::string& personID,
  ...
  content.writeUnsignedByte(TYPE_DOUBLE);
  content.writeDouble(width);
  myParent.send_commandSetValue(CMD_SET_PERSON_VARIABLE, VAR_WIDTH, personID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_PERSON_VARIABLE);
}
...
void TraCIAPI::PersonScope::setHeight(const std::string& personID,
  ...
  content.writeUnsignedByte(TYPE_DOUBLE);
  content.writeDouble(height);
  myParent.send_commandSetValue(CMD_SET_PERSON_VARIABLE, VAR_HEIGHT, personID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_PERSON_VARIABLE);
}
...
void TraCIAPI::PersonScope::setMinGap(const std::string& personID,
  ...
  content.writeUnsignedByte(TYPE_DOUBLE);
  content.writeDouble(minGap);
  myParent.send_commandSetValue(CMD_SET_PERSON_VARIABLE, VAR_MINGAP, personID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_PERSON_VARIABLE);
}
...
void TraCIAPI::PersonScope::setColor(const std::string& personID,
  ...
  content.writeUnsignedByte(c.b);
  content.writeUnsignedByte(c.a);
  myParent.send_commandSetValue(CMD_SET_PERSON_VARIABLE, VAR_COLOR, personID,
                                content);
  tcpip::Storage inMsg;
  myParent.check_resultState(inMsg, CMD_SET_PERSON_VARIABLE);
}

std::string TraCIAPI::TraCIScopeWrapper::getParameter(
    const std::string& objectID, const std::string& key) const {
  tcpip::Storage content;
  content.writeByte(TYPE_STRING);
...
void TraCIAPI::TraCIScopeWrapper::subscribe(const std::string& objID,
  ...
  myParent.send_commandSubscribeObjectVariable(mySubscribeID, objID, beginTime,
                                               endTime, vars);
  tcpip::Storage inMsg;
...
void TraCIAPI::TraCIScopeWrapper::subscribeContext(const std::string& objID,
                                                   int domain, double range,
                                                   const std::vector<int>& vars,
                                                   double beginTime,
                                                   double endTime) const {
  myParent.send_commandSubscribeObjectContext(
      myContextSubscribeID, objID, beginTime, endTime, domain, range, vars);
  tcpip::Storage inMsg;
...
const libsumo::TraCIResults TraCIAPI::TraCIScopeWrapper::getSubscriptionResults(
...
TraCIAPI::TraCIScopeWrapper::getContextSubscriptionResults(
    const std::string& objID) const {
  if (myContextSubscriptionResults.find(objID) !=
      myContextSubscriptionResults.end()) {
