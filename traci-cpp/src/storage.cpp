 ...
 */
int Storage::readUnsignedByte() { return static_cast<int>(readChar()); }
 ...
 * with MSBF-order
 *
 * @return the unspoiled integer value (between -2.147.483.648
 * and 2.147.483.647)
 */
int Storage::readInt() {
  int value = 0;
...
void Storage::writeStorage(tcpip::Storage &other) {
  ...
  store.insert<StorageType::const_iterator>(store.end(), other.iter_,
                                            other.store.end());
  iter_ = store.begin();
}
...
void Storage::checkReadSafe(unsigned int num) const {
  if (std::distance(iter_, store.end()) < static_cast<int>(num)) {
    ...
    msg << "tcpip::Storage::readIsSafe: want to read " << num
        << " bytes from Storage, "
...
void Storage::readByEndianess(unsigned char *array, int size) {
  checkReadSafe(size);
  if (bigEndian_) {
    for (int i = 0; i < size; ++i) array[i] = readCharUnsafe();
  } else {
    for (int i = size - 1; i >= 0; --i) array[i] = readCharUnsafe();
  }
}
...
std::string Storage::hexDump() const {
  ...
  // 'showbase' inserts "0x", 'internal' makes leading '0' appear between "0x"
  // and hex digits
  dump.setf(std::ostream::hex | std::ostream::showbase |
            std::ostream::internal);
  dump.fill('0');

  for (StorageType::const_iterator it = store.begin(); it != store.end();
       ++it) {
    // insert spaces between values
    if (it != store.begin()) dump << "  ";
