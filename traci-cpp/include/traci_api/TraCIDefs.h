...
#define LIBSUMO_SUBSCRIPTION_API                                              \
  static void subscribe(const std::string& objID,                             \
                        const std::vector<int>& vars = std::vector<int>(),    \
                        double beginTime = INVALID_DOUBLE_VALUE,              \
                        double endTime = INVALID_DOUBLE_VALUE);               \
  static void subscribeContext(                                               \
      const std::string& objID, int domain, double range,                     \
      const std::vector<int>& vars = std::vector<int>(),                      \
      double beginTime = INVALID_DOUBLE_VALUE,                                \
      double endTime = INVALID_DOUBLE_VALUE);                                 \
...
#define LIBSUMO_SUBSCRIPTION_IMPLEMENTATION(CLASS, DOMAIN)                     \
  void CLASS::subscribe(const std::string& objID,                              \
                        const std::vector<int>& vars, double beginTime,        \
                        double endTime) {                                      \
    libsumo::Helper::subscribe(CMD_SUBSCRIBE_##DOMAIN##_VARIABLE, objID, vars, \
                               beginTime, endTime);                            \
  }                                                                            \
  void CLASS::subscribeContext(const std::string& objID, int domain,           \
                               double range, const std::vector<int>& vars,     \
                               double beginTime, double endTime) {             \
    libsumo::Helper::subscribe(CMD_SUBSCRIBE_##DOMAIN##_CONTEXT, objID, vars,  \
                               beginTime, endTime, domain, range);             \
  }                                                                            \
  const SubscriptionResults CLASS::getAllSubscriptionResults() {               \
    return mySubscriptionResults;                                              \
  }                                                                            \
  const TraCIResults CLASS::getSubscriptionResults(const std::string& objID) { \
    return mySubscriptionResults[objID];                                       \
  }                                                                            \
  const ContextSubscriptionResults CLASS::getAllContextSubscriptionResults() { \
    return myContextSubscriptionResults;                                       \
  }                                                                            \
  const SubscriptionResults CLASS::getContextSubscriptionResults(              \
      const std::string& objID) {                                              \
...
class TraCIException : public std::runtime_error {
 public:
  /** constructor */
  TraCIException(std::string what) : std::runtime_error(what) {}
};
...
struct TraCIColor : TraCIResult {
  std::string getString() {
    std::ostringstream os;
    os << "TraCIColor(" << (int)r << "," << (int)g << "," << (int)b << ","
       << (int)a << ")";
    return os.str();
  }
...
class TraCIPhase {
 public:
  TraCIPhase() {}
  TraCIPhase(const double _duration, const std::string& _state,
             const double _minDur = INVALID_DOUBLE_VALUE,
             const double _maxDur = INVALID_DOUBLE_VALUE, const int _next = -1)
      : duration(_duration),
        state(_state),
        minDur(_minDur),
        maxDur(_maxDur),
...
class TraCILogic {
 public:
  TraCILogic() {}
  TraCILogic(const std::string& _programID, const int _type,
...
class TraCILink {
 public:
  TraCILink(const std::string& _from, const std::string& _via,
...
class TraCIConnection {
 public:
  TraCIConnection() {}  // this is needed by SWIG when building a vector of this
                        // type, please don't use it
  TraCIConnection(const std::string& _approachedLane, const bool _hasPrio,
                  const bool _isOpen, const bool _hasFoe,
                  const std::string _approachedInternal,
                  const std::string _state, const std::string _direction,
                  const double _length)
      : approachedLane(_approachedLane),
        hasPrio(_hasPrio),
        isOpen(_isOpen),
        hasFoe(_hasFoe),
        approachedInternal(_approachedInternal),
        state(_state),
...
struct TraCIBestLanesData {
  ...
  /// @brief The sequence of lanes that best allows continuing the route without
  /// lane change
