...
class TraCIAPI {
 public:
  ...
  /// @brief Let sumo load a simulation using the given command line like
  /// options.
  void load(const std::vector<std::string>& args);

  /// @name Atomar getter
  /// @{
  int getUnsignedByte(int cmd, int var, const std::string& id,
                      tcpip::Storage* add = 0);
  int getByte(int cmd, int var, const std::string& id, tcpip::Storage* add = 0);
  int getInt(int cmd, int var, const std::string& id, tcpip::Storage* add = 0);
  double getDouble(int cmd, int var, const std::string& id,
                   tcpip::Storage* add = 0);
  libsumo::TraCIPositionVector getPolygon(int cmd, int var,
                                          const std::string& id,
                                          tcpip::Storage* add = 0);
  ...
  std::string getString(int cmd, int var, const std::string& id,
                        tcpip::Storage* add = 0);
  std::vector<std::string> getStringVector(int cmd, int var,
                                           const std::string& id,
                                           tcpip::Storage* add = 0);
   ...
   *
   * Must be derived by interfaces which implement access methods to certain
   * object types
   */
  class TraCIScopeWrapper {
   public:
    ...
    TraCIScopeWrapper(TraCIAPI& parent, int cmdGetID, int cmdSetID,
                      int subscribeID, int contextSubscribeID)
        : myParent(parent),
          myCmdGetID(cmdGetID),
          myCmdSetID(cmdSetID),
    ...
    std::string getParameter(const std::string& objectID,
                             const std::string& key) const;

    /// @brief set generic paramter
    void setParameter(const std::string& objectID, const std::string& key,
                      const std::string& value) const;

    void subscribe(const std::string& objID, const std::vector<int>& vars,
                   double beginTime, double endTime) const;
    void subscribeContext(const std::string& objID, int domain, double range,
                          const std::vector<int>& vars, double beginTime,
                          double endTime) const;

    const libsumo::SubscriptionResults getAllSubscriptionResults() const;
    const libsumo::TraCIResults getSubscriptionResults(
        const std::string& objID) const;

    const libsumo::ContextSubscriptionResults getAllContextSubscriptionResults()
        const;
    const libsumo::SubscriptionResults getContextSubscriptionResults(
        const std::string& objID) const;

    // the following are only for internal use
    void clearSubscriptionResults();
    libsumo::SubscriptionResults& getModifiableSubscriptionResults();
    libsumo::SubscriptionResults& getModifiableContextSubscriptionResults(
  ...
  class EdgeScope : public TraCIScopeWrapper {
   public:
    EdgeScope(TraCIAPI& parent)
        : TraCIScopeWrapper(parent, CMD_GET_EDGE_VARIABLE,
                            CMD_SET_EDGE_VARIABLE, CMD_SUBSCRIBE_EDGE_VARIABLE,
    ...
    std::vector<std::string> getLastStepVehicleIDs(
    ...
    void adaptTraveltime(
        const std::string& edgeID, double time, double beginSeconds = 0.,
        double endSeconds = std::numeric_limits<double>::max()) const;
    void setEffort(
        const std::string& edgeID, double effort, double beginSeconds = 0.,
        double endSeconds = std::numeric_limits<double>::max()) const;
  ...
  class GUIScope : public TraCIScopeWrapper {
   public:
    GUIScope(TraCIAPI& parent)
        : TraCIScopeWrapper(parent, CMD_GET_GUI_VARIABLE, CMD_SET_GUI_VARIABLE,
    ...
    libsumo::TraCIPosition getOffset(
        const std::string& viewID = DEFAULT_VIEW) const;
    std::string getSchema(const std::string& viewID = DEFAULT_VIEW) const;
    libsumo::TraCIPositionVector getBoundary(
        const std::string& viewID = DEFAULT_VIEW) const;
    void setZoom(const std::string& viewID, double zoom) const;
    void setOffset(const std::string& viewID, double x, double y) const;
    void setSchema(const std::string& viewID,
                   const std::string& schemeName) const;
    void setBoundary(const std::string& viewID, double xmin, double ymin,
                     double xmax, double ymax) const;
    void screenshot(const std::string& viewID, const std::string& filename,
                    const int width = -1, const int height = -1) const;
    void trackVehicle(const std::string& viewID,
  ...
  class InductionLoopScope : public TraCIScopeWrapper {
   public:
    InductionLoopScope(TraCIAPI& parent)
        : TraCIScopeWrapper(parent, CMD_GET_INDUCTIONLOOP_VARIABLE, -1,
    ...
    std::vector<libsumo::TraCIVehicleData> getVehicleData(
        const std::string& loopID) const;
  ...
  class JunctionScope : public TraCIScopeWrapper {
   public:
    JunctionScope(TraCIAPI& parent)
        : TraCIScopeWrapper(
              parent, CMD_GET_JUNCTION_VARIABLE, CMD_SET_JUNCTION_VARIABLE,
              CMD_SUBSCRIBE_JUNCTION_VARIABLE, CMD_SUBSCRIBE_JUNCTION_CONTEXT) {
    }
  ...
  class LaneScope : public TraCIScopeWrapper {
   public:
    LaneScope(TraCIAPI& parent)
        : TraCIScopeWrapper(parent, CMD_GET_LANE_VARIABLE,
                            CMD_SET_LANE_VARIABLE, CMD_SUBSCRIBE_LANE_VARIABLE,
    ...
    std::vector<libsumo::TraCIConnection> getLinks(
  ...
  class LaneAreaScope : public TraCIScopeWrapper {
   public:
    LaneAreaScope(TraCIAPI& parent)
        : TraCIScopeWrapper(parent, CMD_GET_LANEAREA_VARIABLE, -1,
  ...
  class MeMeScope : public TraCIScopeWrapper {
   public:
    MeMeScope(TraCIAPI& parent)
        : TraCIScopeWrapper(parent, CMD_GET_MULTIENTRYEXIT_VARIABLE, -1,
  ...
  class POIScope : public TraCIScopeWrapper {
   public:
    POIScope(TraCIAPI& parent)
        : TraCIScopeWrapper(parent, CMD_GET_POI_VARIABLE, CMD_SET_POI_VARIABLE,
    ...
    void add(const std::string& poiID, double x, double y,
             const libsumo::TraCIColor& c, const std::string& type,
             int layer) const;
  ...
  class PolygonScope : public TraCIScopeWrapper {
   public:
    PolygonScope(TraCIAPI& parent)
        : TraCIScopeWrapper(
              parent, CMD_GET_POLYGON_VARIABLE, CMD_SET_POLYGON_VARIABLE,
              CMD_SUBSCRIBE_POLYGON_VARIABLE, CMD_SUBSCRIBE_POLYGON_CONTEXT) {}
    ...
    void setType(const std::string& polygonID,
                 const std::string& setType) const;
    void setShape(const std::string& polygonID,
                  const libsumo::TraCIPositionVector& shape) const;
    void setColor(const std::string& polygonID,
                  const libsumo::TraCIColor& c) const;
    void setLineWidth(const std::string& polygonID,
                      const double lineWidth) const;
    void add(const std::string& polygonID,
             const libsumo::TraCIPositionVector& shape,
             const libsumo::TraCIColor& c, bool fill, const std::string& type,
             int layer) const;
  ...
  class RouteScope : public TraCIScopeWrapper {
   public:
    RouteScope(TraCIAPI& parent)
        : TraCIScopeWrapper(
              parent, CMD_GET_ROUTE_VARIABLE, CMD_SET_ROUTE_VARIABLE,
  ...
  class SimulationScope : public TraCIScopeWrapper {
   public:
    ...
    double getDistanceRoad(const std::string& edgeID1, double pos1,
                           const std::string& edgeID2, double pos2,
  ...
  class TrafficLightScope : public TraCIScopeWrapper {
   public:
    TrafficLightScope(TraCIAPI& parent)
        : TraCIScopeWrapper(parent, CMD_GET_TL_VARIABLE, CMD_SET_TL_VARIABLE,
    ...
    std::vector<libsumo::TraCILogic> getCompleteRedYellowGreenDefinition(
        const std::string& tlsID) const;
    std::vector<std::string> getControlledLanes(const std::string& tlsID) const;
    std::vector<std::vector<libsumo::TraCILink> > getControlledLinks(
        const std::string& tlsID) const;
    ...
    void setRedYellowGreenState(const std::string& tlsID,
                                const std::string& state) const;
    void setPhase(const std::string& tlsID, int index) const;
    void setProgram(const std::string& tlsID,
                    const std::string& programID) const;
    void setPhaseDuration(const std::string& tlsID, double phaseDuration) const;
    void setCompleteRedYellowGreenDefinition(
  ...
  class VehicleTypeScope : public TraCIScopeWrapper {
   public:
    VehicleTypeScope(TraCIAPI& parent)
        : TraCIScopeWrapper(parent, CMD_GET_VEHICLETYPE_VARIABLE,
    ...
    void setVehicleClass(const std::string& typeID,
    ...
    void setEmissionClass(const std::string& typeID,
                          const std::string& clazz) const;
    void setShapeClass(const std::string& typeID,
                       const std::string& shapeClass) const;
    ...
    void setColor(const std::string& typeID,
                  const libsumo::TraCIColor& c) const;
    void setMinGapLat(const std::string& typeID, double minGapLat) const;
    void setMaxSpeedLat(const std::string& typeID, double speed) const;
    void setLateralAlignment(const std::string& typeID,
                             const std::string& latAlignment) const;
    void copy(const std::string& origTypeID,
  ...
  class VehicleScope : public TraCIScopeWrapper {
   public:
    VehicleScope(TraCIAPI& parent)
        : TraCIScopeWrapper(
              parent, CMD_GET_VEHICLE_VARIABLE, CMD_SET_VEHICLE_VARIABLE,
              CMD_SUBSCRIBE_VEHICLE_VARIABLE, CMD_SUBSCRIBE_VEHICLE_CONTEXT) {}
    ...
    enum VehicleSignal {
      ...
      SIGNAL_RESET = -1, /*< sending a negative signal resets all signals to
                            their computed values immediately */
    };
    ...
    std::vector<std::string> getPersonIDList(
    ...
    std::vector<libsumo::TraCINextTLSData> getNextTLS(
        const std::string& vehID) const;
    std::vector<libsumo::TraCIBestLanesData> getBestLanes(
        const std::string& vehicleID) const;
    std::pair<std::string, double> getLeader(const std::string& vehicleID,
                                             double dist) const;
    int getRoutingMode(const std::string& vehicleID) const;
    std::pair<int, int> getLaneChangeState(const std::string& vehicleID,
                                           int direction) const;
    /// @}
    ...
    void add(const std::string& vehicleID, const std::string& routeID,
             ...
             std::string depart = "-1", const std::string& departLane = "first",
             ...
             const std::string& line = "", int personCapacity = 0,
             int personNumber = 0) const;

    void changeTarget(const std::string& vehicleID,
                      const std::string& edgeID) const;
    void changeLane(const std::string& vehicleID, int laneIndex,
                    double duration) const;
    void changeLaneRelative(const std::string& vehicleID, int laneChange,
                            double duration) const;
    void changeSublane(const std::string& vehicleID, double latDist) const;
    void setRouteID(const std::string& vehicleID,
                    const std::string& routeID) const;
    void setRoute(const std::string& vehicleID,
                  const std::vector<std::string>& edge) const;
    void rerouteTraveltime(const std::string& vehicleID,
                           bool currentTravelTimes = true) const;
    void moveTo(const std::string& vehicleID, const std::string& laneID,
                double position) const;
    void moveToXY(const std::string& vehicleID, const std::string& edgeID,
                  const int lane, const double x, const double y,
                  const double angle, const int keepRoute) const;
    void slowDown(const std::string& vehicleID, double speed,
                  double duration) const;
    void openGap(const std::string& vehicleID, double newTau, double duration,
                 double changeRate, double maxDecel) const;
    void setSpeed(const std::string& vehicleID, double speed) const;
    void setSpeedMode(const std::string& vehicleID, int mode) const;
    void setStop(const std::string vehicleID, const std::string edgeID,
                 const double endPos = 1., const int laneIndex = 0,
    ...
    void remove(const std::string& vehicleID,
                char reason = REMOVE_VAPORIZED) const;
    void setColor(const std::string& vehicleID,
                  const libsumo::TraCIColor& c) const;
    void setLine(const std::string& vehicleID, const std::string& line) const;
    void setVia(const std::string& vehicleID,
    ...
    /// @{
    void setShapeClass(const std::string& vehicleID,
                       const std::string& clazz) const;
    void setEmissionClass(const std::string& vehicleID,
                          const std::string& clazz) const;
  ...
  class PersonScope : public TraCIScopeWrapper {
   public:
    ...
    std::vector<std::string> getEdges(const std::string& personID,
                                      int nextStageIndex = 0) const;
    ...
    void add(const std::string& personID, const std::string& edgeID, double pos,
             double depart = DEPARTFLAG_NOW,
             const std::string typeID = "DEFAULT_PEDTYPE");
    void appendWaitingStage(const std::string& personID, double duration,
                            const std::string& description = "waiting",
                            const std::string& stopID = "");
    void appendWalkingStage(const std::string& personID,
                            const std::vector<std::string>& edges,
                            double arrivalPos, double duration = -1,
                            double speed = -1, const std::string& stopID = "");
    void appendDrivingStage(const std::string& personID,
                            const std::string& toEdge, const std::string& lines,
                            const std::string& stopID = "");
 ...
 protected:
   ...
   */
  void send_commandGetVariable(int domID, int varID, const std::string& objID,
   ...
   */
  void send_commandSetValue(int domID, int varID, const std::string& objID,
   ...
   */
  void send_commandSubscribeObjectVariable(int domID, const std::string& objID,
   ...
   * @param[in] vars The variables to subscribe
   */
  void send_commandSubscribeObjectContext(int domID, const std::string& objID,
                                          double beginTime, double endTime,
                                          int domain, double range,
                                          const std::vector<int>& vars) const;
  /// @}

  void send_commandMoveToXY(const std::string& vehicleID,
   ...
   * @param[in] ignoreCommandId Whether the returning command id shall be
   * validated
   * @param[in] acknowledgement Pointer to an existing string into which the
   * acknowledgement message shall be inserted
   */
  void check_resultState(tcpip::Storage& inMsg, int command,
                         bool ignoreCommandId = false,
                         std::string* acknowledgement = 0) const;

  /** @brief Validates the result state of a command
   * @return The command Id
   */
  int check_commandGetResult(tcpip::Storage& inMsg, int command,
                             int expectedType = -1,
                             bool ignoreCommandId = false) const;

  void processGET(tcpip::Storage& inMsg, int command, int expectedType,
                  bool ignoreCommandId = false) const;
  /// @}

  void readVariableSubscription(int cmdId, tcpip::Storage& inMsg);
  void readContextSubscription(int cmdId, tcpip::Storage& inMsg);
  void readVariables(tcpip::Storage& inMsg, const std::string& objectID,
                     int variableCount, libsumo::SubscriptionResults& into);
